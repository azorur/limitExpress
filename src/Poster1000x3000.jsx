...
export default function PosterResponsive() {
  const brand = "극한 퀵 서비스";
  const phone = "1844-1025";

  const h = (px) => ({ fontSize: px, fontWeight: 800, lineHeight: 1.2, margin: 0 });
  const t = (px, color="#475569") => ({ fontSize: px, color, margin: 0 });
  const card = { borderRadius: 16, border: "1px solid #e9eef7", background: "#fff", boxShadow: "0 8px 30px rgba(13,51,126,.12)" };
  const center = "d-flex flex-column align-items-center text-center";

  return (
    <div 
      className={`${center}`} 
      style={{ width:"100%", maxWidth:1000, margin:"0 auto", background:"#fff", color:"#0f172a", fontFamily:"system-ui,-apple-system,Segoe UI,Roboto,Noto Sans KR,Arial" }}
    >
      <style>{`
        ...
        .card-soft:hover { transform: translateY(-2px); box-shadow:0 20px 60px rgba(13,51,126,.18); }
      ...
      >
        <div 
          ...
          style={{ background: "linear-gradient(135deg, #132d83ff, #729dfbff)" }}
        />
        <div 
          className="position-absolute w-100 h-100" 
          style={{
        ...
        <div className="position-relative container py-4 d-flex flex-column align-items-center justify-content-center">
          <img src="/title.png" alt="극한퀵서비스 로고" style={{ maxWidth:"280px", marginBottom:"12px" }} />

          <img
            src="/call.png"
            alt="전화걸기"
            style={{ maxWidth:"280px", marginBottom:"12px", cursor:"pointer" }}
            onClick={() => {
              const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
              if (isMobile) window.location.href = `tel:${phone}`;
              else alert("전화 연결은 모바일 기기에서만 가능합니다.");
            }}
          />

          <p className="mt-2" style={t(18, "#dbeafe")}>빠른 배차 · 전국 당일배송 · 24시 연중무휴</p>

          <div className="row g-2 mt-2 justify-content-center w-100">
      ...
      <section className={`${center} container`} style={{ padding:"28px 24px 0" }}>
        <h2 style={h(32)}>어떤 화물도, 맞춤 차량으로</h2>
        <p className="mt-1" style={t(18, "#607d8b")}>오토바이 · 다마스 · 라보 즉시 배차</p>
          ...
          {[
            ...
            <div key={i} className="col-12 col-md-4">
              <div className={`p-3 h-100 ${center} card-soft`} style={card}>
                <p className="mt-1 text-primary" style={{ fontSize:20, fontWeight:800 }}>{v.title}</p>
                <p className="mt-1" style={t(14)}>{v.sub}</p>
                <ul className="mt-2 text-start" style={{ fontSize:14, paddingLeft:20 }}>
                  {v.bullets.map(b => <li key={b} className="mb-1">{b}</li>)}
                </ul>
      ...
      {/* =================== 선택 이유 =================== */}
      <section className={`${center} container`} style={{ padding:"28px 24px 0" }}>
        <h3 style={h(28)}>왜 {brand}인가요?</h3>
        <p className="mt-1" style={t(14, "#64748b")}>실제 운송 운영 기준으로 투명하고 빠르게 대응합니다.</p>
          ...
          {[
            ...
            <div key={i} className="col-12 col-md-4">
              <div className={`p-3 ${center} card-soft`} style={card}>
                <p className="mb-1 text-primary" style={{ fontSize:18, fontWeight:800 }}>{title}</p>
                <p className="mb-0" style={t(14, "#37474f")}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={i} className="col-6 col-md-2">
              <div className={`p-3 ${center}`} style={{ ...card, background:"#f8fafc" }}>
                <span 
                  ...
                  style={{ width: 28, height: 28, background:"#1d4ed8", fontWeight: 800 }}
                >
                  {i+1}
                </span>
                <p className="mb-0 fw-bold" style={t(14, "#0f172a")}>{title}</p>
                <p className="mb-0" style={t(12, "#607d8b")}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className={`${center} container`} style={{ padding:"28px 24px 0" }}>
        ...
        {[
          ...
          <div key={idx} className="p-3 mt-2 w-100" style={{ ...card, background:"#f8fafc" }}>
            <p className="mb-1 text-primary" style={{ fontSize:16, fontWeight:800 }}>Q. {q}</p>
            <p className="mb-0" style={t(14, "#0f172a")}>A. {a}</p>
          </div>
        ))}
      </section>
      ...
      <footer 
        ...
        style={{ width:"100%", minHeight:280, marginTop:24, color:"#fff", background:"linear-gradient(135deg, #132d83ff, #729dfbff)" }}
      >
        <p className="mb-0" style={t(18, "#e2e8f0")}>지금 배차 상담하세요</p>
        <img src="/title.png" alt="극한퀵서비스 로고" style={{ maxWidth:"240px", marginBottom:"12px" }} />
        <img
          src="/call.png"
          alt="전화걸기"
          style={{ maxWidth:"240px", marginBottom:"12px", cursor:"pointer" }}
          onClick={() => {
            const isMobile = /Android|iPhone|iPad|iPod/i.test(navigator.userAgent);
            if (isMobile) window.location.href = `tel:${phone}`;
            else alert("전화 연결은 모바일 기기에서만 가능합니다.");
